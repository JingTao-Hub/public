...
import "./style.scss";
// 设置企业 key
LicenseManager.setLicenseKey('SHI_UK_on_behalf_of_Jing_Sweden_MultiApp_1Devs6_November_2019__MTU3Mjk5ODQwMDAwMA==e27a8fba6b8b1b40e95ee08e9e0db2cb');
...
export interface ITableProps extends AgGridReactProps {
    ...
    checkboxSelectionProps?: ColGroupDef | ColDef;
...
class Table extends React.Component<ITableProps, any> {
    ...
    onUpdateHeight(refFullscreen = false) {
        try {
            // props 中传递了 height
            if ((this.props.style && this.props.style.height) || this.props.onUpdateHeight === false || this.props.gridAutoHeight) {
                return
            }
            const refTable = this.refTableBody.current;//ReactDOM.findDOMNode(this.ref.current) as HTMLDivElement;
            // 60 是头部 标题栏 高度
            let height = window.innerHeight - refTable.offsetTop - (this.props.paginationProps ? 72 : 28);
            // 全屏 加回 60 高度
            if (refFullscreen || this.isOuter) {
                height += 20;
            }
            if (document.querySelector('.demo-layout-main')) {
                height -= 50;
            }
            ...
            if (this.state.height !== height) {
                this.gridApi && this.gridApi.sizeColumnsToFit && this.gridApi.sizeColumnsToFit();
    ...
    componentDidMount() {
        this.onUpdateHeight();
        // 判断是否是 外部页面
        if (document.body.classList && document.body.classList.contains('demo-layout-outer')) {
            this.isOuter = true;
        }
    ...
    public render() {
        ...
        return (
            <>
                <div ref={this.refTableBody} style={{ height: this.props.gridAutoHeight ? 'auto' : this.state.height, ...style }} className={`demo-ag-grid ${className} ${theme}`}>
